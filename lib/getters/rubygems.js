...
const rubygems = "<img height='16px' width='16px' src='atom://autocomplete-gemfile/resources/rubygems_logo_red.png' />";
const client = {
  searchGems(query) {
...
export default {
  getNames({name}) {
    return client.searchGems(name)
      .then(gems => gems.json())
      .then(gems => gems.map(({name}) => ({text: name, iconHTML: rubygems })));
  },
